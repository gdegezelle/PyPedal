...
if __name__ == "__main__":
    ...
    example = pyp_newclasses.load_pedigree(options_file="new_db.ini")

    db = pyp_db.getCursorSQA(dbdebug=example.kw["database_debug"])
    if db:
        print(
            "Successfully connected to the database %s." % (example.kw["database_name"])
        )
    if pyp_db.tableExistsSQA(db, example.kw["dbtable_name"]):
        print(
            "The table %s exists in %s"
            % (example.kw["dbtable_name"], example.kw["database_name"])
        )
    if not pyp_db.tableExistsSQA(db, "cheesegrommit"):
        print(
            "The table cheesegrommit does not exist in %s"
            % (example.kw["database_name"])
        )
