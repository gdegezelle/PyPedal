...
if __name__ == "__main__":
    ...
    example = pyp_newclasses.load_pedigree(options_file="new_lacy.ini", debug_load=True)
    example.printoptions()
    if example.kw["messages"] == "verbose":
        print("=" * 80)
        print(
            "[INFO]: Calling pyp_metrics.effective_founders_lacy at %s"
            % (pyp_nice_time())
        )
    pyp_metrics.effective_founders_lacy(example)

    # Example taken from Boichard et al. (1997), Figure 2 / Table II.
    example2a = pyp_newclasses.load_pedigree(options_file="new_format.ini")
