...
if __name__ == "__main__":
    ...
    # This is an advanced example that uses configparser to read
    # configurations for multiple pedigrees from the same file.
    from configparser import ConfigParser
    import sys

    config = ConfigParser()
    config.read("new_inbreeding2multiple.ini")
    
    if not config.sections():
        print("[ERROR]: No sections found in new_inbreeding2multiple.ini")
        sys.exit(0)
    
    # Convert ConfigParser sections to dictionaries
    kw_all = {section: dict(config.items(section)) for section in config.sections()}
    
    kw_noinbred = kw_all.get("noinbreeding", {})
    kw_horse = kw_all.get("horse", {})
    
    if not kw_horse:
        print("[ERROR]: 'horse' section not found in configuration file")
        sys.exit(0)

    #     noinbred = pyp_newclasses.loadPedigree(kw_noinbred)
    horse = pyp_newclasses.load_pedigree(kw_horse)
    ...
    horse_inbreeding = pyp_nrm.inbreeding(horse, method="tabular", rels=1)
    print("\nHorse inbreeding: %s" % (horse_inbreeding))
    print("\nHorses wth non-zero coefficients of inbreeding:")
    print("-----------------------------------------------")
    for _h in horse_inbreeding["fx"].keys():
        if horse_inbreeding["fx"][_h] > 0.0:
            print(
                "%s\t\t%s"
                % (horse.pedigree[int(_h) - 1].name, horse_inbreeding["fx"][_h])
            )
