...
# VERSION: originally 2.0.0 (29SEPTEMBER2010); current package version is PyPedal.__version__
...
def rmuller_spy_matrix_pil(
    ...
    if n > width or m > height:
        raise ValueError("Rectangle too big %d %d %d %d" % (n, m, width, height))
    for i in range(n):
...
def rmuller_get_color(a, cmin, cmax):
    ...
    try:
        ...
        a = 0.5  # cmax == cmin
    blue = min((max((4 * (0.75 - a), 0.0)), 1.0))
    red = min((max((4 * (a - 0.25), 0.0)), 1.0))
    green = min((max((4 * math.fabs(a - 0.5) - 1.0, 0)), 1.0))
...
def draw_pedigree(
    ...
    from PyPedal.pyp_utils import string_to_table_name
            ...
            if int(_m.sireID) != pedobj.kw["missing_parent"]:
                if gname:
                    ...
                    if garrow:
                        g.add_edge(
                            pydot.Edge(
                                str(pedobj.pedigree[int(
                                    _m.sireID) - 1].originalID),
                                str(_m.originalID),
                            )
                        )
                    else:
    ...
    # Or test the new subgraph clusters
        ...
        for _g in pedobj.metadata.unique_gen_list:
            ...
            if gshowall:
                sg = pydot.Cluster(
                    graph_name=str(_sg_name), suppress_disconnected=False, simplify=True
                )
            else:
                sg = pydot.Cluster(
                    graph_name=str(_sg_name), suppress_disconnected=True, simplify=True
                )
            for _m in pedobj.pedigree:
...
def plot_founders_by_year(
    pedobj, gfilename="founders_by_year", gtitle="Founders by Birthyear"
):
    """
    founders_by_year() uses matplotlib -- if available on your system -- to produce a
    bar graph of the number (count) of founders in each birthyear.
    ...
    try:
        ...
        if pedobj.kw["messages"] == "verbose":
            print(
                "[ERROR]: pyp_graphics/plot_founders_by_year() was unable to create the plot '%s' (%s.png)."
                % (gtitle, gfilename)
            )
...
def plot_founders_pct_by_year(
    pedobj, gfilename="founders_pct_by_year", gtitle="Founders by Birthyear"
):
    """
    founders_pct_by_year() uses matplotlib -- if available on your system -- to produce a
    line graph of the frequency (percentage) of founders in each birthyear.
    ...
    _freqdict = {}
    for _k in fby.keys():
        _freqdict[_k] = float(fby[_k]) / \
            float(pedobj.metadata.num_unique_founders)
    try:
        import matplotlib
...
def new_draw_pedigree(
    ...
    if colorByUser:
        if "u" not in pedobj.kw["pedformat"]:
            ...
            #
            # matplotlib.cm.get_cmap was deprecated in 3.7 and REMOVED in 3.11,
            # so `plt.cm.get_cmap("Accent", n)` raises AttributeError on any
            # current install. `matplotlib.colormaps[name].resampled(n)` is the
            # documented replacement and is bit-identical to it: verified equal
            # for every entry at n = 1, 2, 3, 5 and 8 under 3.10, where both
            # APIs still exist.
            #
            # This was masked until the webcolors lookup below was removed --
            # the colorByUser tests could not reach this line.
            import matplotlib
    ...
    for _m in pedobj.pedigree:
        ...
        if colorByUser:
            ...
            if colorByUserPalette:
                ...
                # Emit the colormap's own RGB as a hex triplet, which Graphviz
                # accepts directly.
                #
                # This used to go through get_colour_name(), which asked
                # `webcolors` for the nearest CSS colour NAME -- a lossy step:
                # it returned an exact name only when rgb_to_name() happened to
                # match, and otherwise the nearest name by Euclidean RGB
                # distance, which is usually a different colour. It also could
                # not run at all, because it called webcolors.css3_hex_to_names,
                # an API removed long before the mappings were withdrawn
                # entirely in webcolors 24.6.0, and no extra declared the
                # dependency. Nodes now render in the colormap's true colour.
                n.attr["fillcolor"] = "#%02x%02x%02x" % (
                    int(_rgb[0] * 255), int(_rgb[1] * 255), int(_rgb[2] * 255)
                )
        ...
        if int(_m.sireID) != pedobj.kw["missing_parent"]:
            if gname:
                ...
                if gname == "id":
                    ...
                    _sire_edge = pedobj.pedigree[int(
                        _m.sireID) - 1].displayName
                elif gname == "animal_display":
                    _sire_edge = "%s\n%s" % (
                    ...
                    _sire_edge = "%s\n%s" % (
                        pedobj.pedigree[int(_m.sireID) - 1].animalID,
                        pedobj.pedigree[int(_m.sireID) - 1].displayName,
                    )
                else:
                    _sire_edge = pedobj.pedigree[int(_m.sireID) - 1].animalID
        ...
        if int(_m.damID) != pedobj.kw["missing_parent"]:
            if gname:
                ...
                if gname == "id":
                    ...
                    _dam_edge = "%s\n%s" % (
                        pedobj.pedigree[int(_m.damID) - 1].animalID,
                        pedobj.pedigree[int(_m.damID) - 1].displayName,
                    )
                else:
                    _dam_edge = pedobj.pedigree[int(_m.damID) - 1].animalID
...
##
# closest_colour() and get_colour_name() were removed here.
#
# They mapped a Matplotlib colormap RGB triple to the nearest CSS colour NAME,
# for use as a Graphviz fillcolor. Two reasons they are gone:
#
#   1. They could not run. closest_colour() called webcolors.css3_hex_to_names,
#      lowercase -- an API that predates webcolors 1.11.1 (which already used
#      CSS3_HEX_TO_NAMES) and whose raw mappings were withdrawn entirely in
#      24.6.0: "the raw mappings of color names/values are no longer publicly
#      exposed". No installable version of webcolors satisfies that call, and no
#      extra in pyproject.toml declared the dependency, so colorByUser raised in
#      every supported installation. PyPedal 2.0.4 has the identical call and
#      flagged it unresolved in its own CHANGES.txt on 06/27/2022.
#
#   2. The step was lossy. get_colour_name() returned an exact name only when
#      rgb_to_name() happened to match, and otherwise the nearest name by
#      Euclidean RGB distance -- a different colour from the one the colormap
#      chose.
#
# Graphviz accepts #RRGGBB directly, so new_draw_pedigree() now emits the
# colormap's own RGB. Nodes render in their true colour rather than in the
# closest CSS approximation to it: a deliberate cosmetic change, and a more
# faithful rendering. Graphviz accepts #RRGGBB hex colours directly.
